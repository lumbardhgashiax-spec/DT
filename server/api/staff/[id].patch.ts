...
export default defineEventHandler(async (event) => {
  ...
  if (targetId === userId && (!isActive || role !== 'superadmin')) {
    throw createError({ statusCode: 400, statusMessage: 'Nuk mund ta çaktivizoni ose ulni rolin e llogarisë suaj.' })
  }
  ...
  if (target.role === 'superadmin' && target.is_active && (!isActive || role !== 'superadmin')) {
    ...
    if ((count || 0) <= 1) {
      throw createError({ statusCode: 400, statusMessage: 'Duhet të mbetet së paku një superadmin aktiv.' })
    }
  }
