...
export default defineEventHandler(async (event) => {
  ...
  if (reservationsError) throw createError({ statusCode: 500, message: 'Rezervimet e fushes nuk mund te verifikoheshin.' })
  if (reservationsCount) {
    throw createError({ statusCode: 409, message: 'Fusha nuk mund te fshihet sepse ka rezervime te lidhura. Caktivizoje nese nuk do te pranoje rezervime te reja.' })
  }
  ...
  if (imagesError) throw createError({ statusCode: 500, message: 'Fotografite e fushes nuk mund te verifikoheshin.' })

  const { error, count } = await client.from('courts').delete({ count: 'exact' }).eq('id', id)
  if (error) throw createError({ statusCode: error.code === '23503' ? 409 : 400, message: error.code === '23503' ? 'Fusha nuk mund te fshihet sepse ka te dhena te lidhura.' : 'Fusha nuk mund te fshihet.' })
  if (!count) throw createError({ statusCode: 404, message: 'Fusha nuk u gjet.' })
