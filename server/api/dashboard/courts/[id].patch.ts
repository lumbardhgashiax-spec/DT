...
export default defineEventHandler(async (event) => {
  ...
  if (error) {
    throw createError({ statusCode: 400, message: error.code === '23505' ? 'Ekziston tashmë një fushë me këtë emër.' : 'Fusha nuk mund të përditësohej.' })
  }
  if (!data) throw createError({ statusCode: 404, message: 'Fusha nuk u gjet.' })
